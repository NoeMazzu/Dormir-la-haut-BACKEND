...
router.patch("/poiBookMark", async (req, res) => {
  ...
  if (!poi) {
    ...
    await Poi.updateOne({_id:req.body.poiId}, {$addToSet: {favorite: userId._id}});
    return res.json({result: true, message: 'POI marqué comme favori par l\'utilisateur'});
  } else {
    // Si userId._id est déjà présent, ne rien faire
    return res.json({result: false, message: 'POI déjà marqué comme favori par l\'utilisateur'});
  }
});
...
router.patch("/poiUnBookMark", async (req, res) => {
  ...
  if (poi) {
    ...
    await Poi.updateOne({_id:req.body.poiId}, {$pull: {favorite: userId._id}});
    return res.json({result: true, message: 'POI désépinglé par l\'utilisateur'});
  } else {
    // Si userId._id n'est pas présent, ne rien faire
    return res.json({result: false, message: 'POI non épinglé par l\'utilisateur'});
  }
});
